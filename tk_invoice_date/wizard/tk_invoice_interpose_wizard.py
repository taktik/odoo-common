    ...
    def next(self, cr, uid, ids, context=None):
        ...
        if invoice_ids and len(invoice_ids) > 0:
            raise osv.except_osv("Warning",
                                 "You cannot interpose an invoice at this"
                                 " number (%s). There is already an invoice"
                                 " with this number." % number)

        sequence_id = invoice.journal_id.sequence_id.id
        sequences = sequence_obj.read(cr, uid, [sequence_id],
                                      ['company_id', 'implementation',
                                       'number_next', 'prefix', 'suffix',
                                       'padding'])
