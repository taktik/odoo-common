...
class tk_import_wizard_export_sample_with_data(osv.osv_memory):
    ...
    _columns = {
        ...
        'column_ids': fields.many2many('tk_import.column',
                                       'tk_import_column_to_wizard_with_data',
                                       'wizard_id', 'column_id', 'Columns'),
        'key_ids': fields.many2many('tk_import.column',
                                    'tk_import_key_to_wizard_with_data',
                                    'wizard_id', 'key_id', 'Columns'),
    ...
    def iterate(self, cr, uid, ids, context=None):
        ...
        new_model_id = ir_model_obj.search(
            cr, uid, [('model', '=', new_tree['model'])])[0]
    ...
    def default_get(self, cr, uid, field_list, context=None):
        ...
        if context.get('active_model') == 'tk_import.template':
            model_id = model_obj.search(
                cr, uid, [('model', '=', 'tk_import.template')])[0]
            import_model_ids = import_model_obj.search(cr, uid, [
                ('model_id', '=', model_id)])
